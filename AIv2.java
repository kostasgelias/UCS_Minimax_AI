...
//Dhmiourgia tou dentrou.
class StateTree{
	//UCS FUNCTION
	public void UCS(State root,int N ){
		//Current state to be checked.
		State state = root;
		//Filling the blanks in searchField.
		State defaultstate = new State(-1) ;
		...
		while ( searchField_size > 0 ){
			//Checking the g.
			for (int d= state.id_Get() + 1; d < searchField_size;d++){
				
				//Check what state to choose (With the lower g.).
				if (searchField.get(d).GGet() < state.GGet()){
	...
	public void AStar(State root,int N ){
		...
		State state = root;
		//Filling the blanks in searchField.
		State defaultstate = new State(0,1000000) ;
		defaultstate.id_Set(-1) ;
		//If the list is seen before.
		boolean equality=false;
		//For the swap.
